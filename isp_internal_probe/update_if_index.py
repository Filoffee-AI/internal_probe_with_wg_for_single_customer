...
def find_interfaces_for_silverpeak_host(device_id, device_ip, location_id, snmp_str, ssh_username, ssh_password, ssh_port, cust_id, successful_ips, failed_ipsy):
    ...
    try:
        ...
        print(f"{device_ip}: {all_interfaces}")
...
def find_interfaces_for_fortinet_host(device_id, device_ip, location_id, snmp_str, ssh_username, ssh_password, ssh_port, cust_id, successful_ips, failed_ipsy):
    ...
    try:
        ...
        for isp in isps:
            ...
            command = f'get system interface physical'
            try:
                ssh = paramiko.SSHClient()
                ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                ssh.load_system_host_keys()
...
def find_interfaces_for_cisco_l3_switch_interface(device_id, device_ip, location_id, snmp_str, ssh_username, ssh_password, ssh_port, cust_id, successful_ips, failed_ips):
    conn = mysql.connector.connect(
        host=config.mysql_conf["host"],
        user=config.mysql_conf["user"],
        password=config.mysql_conf["password"],
...
def     find_interfaces_for_palo_alto_host(device_id, device_ip, location_id, snmp_str, ssh_username, ssh_password, ssh_port, cust_id, snmp_version, security_level, auth_type, auth_password, privacy_type, privacy_password, successful_ips, failed_ips):
    try:
        # Establish MySQL connection
        conn = mysql.connector.connect(**config.mysql_conf)
        cursor = conn.cursor()

        # Retrieve ISP details for the given location
        isp_sql = f"SELECT isp_wan_id, internal_ip FROM fn_isp_details WHERE location_id='{location_id}'"
        cursor.execute(isp_sql)
        isps = cursor.fetchall()
        # Iterate over ISP details
        for isp in isps:
            isp_wan_id, ip = isp
            command = f"show interface all | match {ip}"
            ssh_client = paramiko.SSHClient()
            ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh_client.connect(device_ip, port=ssh_port, username=ssh_username, password=ssh_password, timeout=15)
            print(f"SSH connection to {device_ip} successful.")
            ...
            interface_line = re.search(r'(\S+/\d+)', output)

            if interface_line:
                # Extract the interface name from the line
                interface_name = interface_line.group(1)
                snmp_cmd = []  # Initialize cmd to an empty list

                if snmp_version == 2:
                    snmp_cmd = ['snmpwalk', '-v2c', '-c', snmp_str, device_ip, 'IF-MIB::ifName']
                elif snmp_version == 3:
                    if security_level == "0":
                        snmp_cmd = ['snmpwalk', '-v3', '-u', snmp_str, device_ip, 'IF-MIB::ifName']
                    elif security_level == "1":
                        snmp_cmd = ['snmpwalk', '-v3', '-l', 'authNoPriv', '-u', snmp_str, '-a', auth_type, '-A', auth_password, device_ip, 'IF-MIB::ifName']
                    elif security_level == "2":
                        snmp_cmd = ['snmpwalk', '-v3', '-l', 'authPriv', '-u', snmp_str, '-a', auth_type, '-A', auth_password, '-x', privacy_type, '-X', privacy_password, device_ip, 'IF-MIB::ifName']

                ifname_output = subprocess.run(snmp_cmd, stdout=subprocess.PIPE)
                ifname_output_lines = ifname_output.stdout.decode('utf-8').split('\n')
                if_index = None
                for line in ifname_output_lines:
                    if interface_name in line:
                        match = re.search(r'IF-MIB::ifName\.(\d+) = STRING: {}'.format(re.escape(interface_name)), line)
                        if match:
                            if_index = match.group(1)
                            break
                
                if if_index != None:
                    update_if_det_sql = f"UPDATE `fn_isp_details` SET `if_index`='{if_index}', `if_name`='{interface_name}', `edge_device_id`='{device_id}', `firewall_ip`='{device_ip}' WHERE  `isp_wan_id`='{isp_wan_id}'"
                    cursor.execute(update_if_det_sql)
                    conn.commit()
                    print(f"Updated ifindex for {ip} with index {if_index} and if_name={interface_name}")
                else:
                    update_if_det_sql = f"UPDATE `fn_isp_details` SET `if_index`='{if_index}', `if_name`='{interface_name}', `edge_device_id`='{device_id}', `firewall_ip`='{device_ip}' WHERE  `isp_wan_id`='{isp_wan_id}'"
                    cursor.execute(update_if_det_sql)
                    conn.commit()
                    print(f"Updated ifindex for {ip} with index {if_index} and if_name={interface_name}")
                    print(f"Interface name {interface_name} not found.")

                    

    except mysql.connector.Error as ex:
        print(f"MySQL error occurred: {ex}")
...
def update_ifindex(device_id, device_ip, location_id, snmp_str, ssh_username, ssh_password, ssh_port, vendor, type, cust_id, snmp_version, security_level, auth_type, auth_password, privacy_type, privacy_password,successful_ips, failed_ips):
    ...
    if vendor == "Cisco":
        if type == "L3-Switch":
            find_interfaces_for_cisco_l3_switch_interface(device_id, device_ip, location_id, snmp_str, ssh_username, ssh_password, ssh_port, cust_id, successful_ips, failed_ips)
        if type == "Router":
    ...
    if vendor == "Palo Alto":
        if type == "Firewall":
            find_interfaces_for_palo_alto_host(device_id, device_ip, location_id, snmp_str, ssh_username, ssh_password, ssh_port, cust_id, snmp_version, security_level, auth_type, auth_password, privacy_type, privacy_password, successful_ips, failed_ips)
...
if __name__ == "__main__":
    ...
    process_list = []
    device_sql = "SELECT device_id, device_ip, location_id, snmp_str, ssh_username, ssh_password, ssh_port, vendor, type, cust_id, snmp_version, security_level, auth_type, auth_password, privacy_type, privacy_password FROM fn_edge_devices"
    ...
    for row in result:
        ...
        auth_type = row[12]
        auth_password = row[13]
        privacy_type = row[14]
        privacy_password = row[15]
        process = multiprocessing.Process(target=update_ifindex, args=(device_id, device_ip, location_id, snmp_str, ssh_username, ssh_password, ssh_port, vendor, type, cust_id, snmp_version, security_level, auth_type, auth_password, privacy_type, privacy_password,successful_ips, failed_ips,))
