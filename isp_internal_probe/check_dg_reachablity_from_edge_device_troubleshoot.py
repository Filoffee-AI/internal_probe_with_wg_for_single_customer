...
def check_dg_status(vendor_id, isp_wan_id, device_id):
    main_dict = {'dg_code': -1}
    try:
        ...
        cursor.execute("""SELECT internal_ip, default_gateway from fn_isp_details WHERE isp_wan_id = '{}';""".format(isp_wan_id))
        ...
        if vendor_id == 2: 
            ...
            dg_dict = {}
            commands = [f"ping {dg_ip} source {wan_ip} count 4"]
            output = ""
            for cmd in commands:
                channel.send(cmd + "\n")
                time.sleep(2)  
            ...
            dg_dict = {}
            commands = [f"ping {dg_ip} -I {wan_ip} -c 4"]
            time.sleep(3)
            ...
            if type == "Router":
                ...
                dg_dict = {}
                commands = [f"ping {dg_ip} source {wan_ip}"]
                output = ""
                for cmd in commands:
                    channel.send(cmd + "\n")
                    time.sleep(2)  
            ...
            dg_dict = {}
            commands = [f"ping source {wan_ip} count 4 host {dg_ip}"]
            output = ""
            for cmd in commands:
                channel.send(cmd + "\n")
                time.sleep(4)  
        ...
        main_dict['dg_code'] = -1
        main_dict["dg"] = {"packet_loss": 100, "latency": "0"}
...
if __name__ == "__main__":
    ...
    check_dg_status(vendor_id, isp_wan_id, device_id)
