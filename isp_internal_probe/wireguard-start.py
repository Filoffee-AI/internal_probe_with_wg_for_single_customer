import subprocess
import ipaddress
ping_command = "ping -c 1 10.80.0.100"
...
# Run the command to get a list of used IP addresses
  command = "sshpass -p 'AHJXHZ@vrven899' ssh -o StrictHostKeyChecking=no root@wg.linkeye.io  wg show wg0 allowed-ips | grep -E -o '([0-9]{1,3}[\.]){3}[0-9]{1,3}'"
...
# Iterate through the range of IP addresses in the subnet
  subnet = ipaddress.ip_network('10.80.8.0/21')
...
# Run the command
  ...
  set_peer_command = f"sshpass -p 'AHJXHZ@vrven899' ssh -o StrictHostKeyChecking=no root@wg.linkeye.io sudo wg set wg0 peer {pub_key} allowed-ips {available_ip} persistent-keepalive 25"
  ...
  new_password = "L1nk@2023"
  start_root_command = f"echo 'root:{new_password}' | chpasswd"
